...
for idx, row in cors.iterrows():
  # print row
  print(idx+'&'+'&'.join([str(x)[:5] for x in row])+'\\\\')
...
best_tree = ncv_set.perform_CV(BinClassifier(severity=False,
                               proc=tree.DecisionTreeClassifier( criterion="entropy", class_weight='balanced')), sparse=False)
print(best_tree)

best_tree = ncv_set.perform_CV(BinClassifier(severity=False,
                               proc=tree.DecisionTreeClassifier(criterion="gini", class_weight='balanced')), sparse=False)
print(best_tree)
print('Forest')
best_tree = ncv_set.perform_CV(BinClassifier(severity=False,
                               proc=ensemble.RandomForestClassifier(criterion="entropy", class_weight='balanced')), sparse=False)
print(best_tree)

best_tree = ncv_set.perform_CV(BinClassifier(severity=False,
                               proc=ensemble.RandomForestClassifier(criterion="gini", class_weight='balanced')), sparse=False)
print(best_tree)
...
'''
print('Best LDA:')
best_lda = ncv_set.perform_CV(BinClassifier(proc=LinearDiscriminantAnalysis(shrinkage=0.8, priors=(0.029,0.961), solver="lsqr")))
print(best_lda)
print(best_lda.getFeatures())

print('Best SVM:')
best_svm = ncv_set.perform_CV(BinClassifier(proc=svm.SVC(probability=True, kernel="linear", C=0.005, class_weight='balanced'), severity=True))
print(best_svm)
